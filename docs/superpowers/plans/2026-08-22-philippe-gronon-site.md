...
- **Task 30, part 2, amends this bullet: the curation flag is back.** This bullet used to read "there is no curation flag" and explained that an earlier `featured` boolean was removed as unnecessary. The client asked for it back: `Article.featured` (works only) drives homepage slideshow membership, toggled per-article in the admin list, ordered by the article list's own manual `position` (no second ordering mechanism). `GET /home` queries featured works first and falls back to the most recent published works with a cover when nothing is featured, so the slideshow can never render empty. Each work still has exactly one `cover` image, which still serves both the archive grid and the slideshow slide -- that part of the original reasoning is unchanged, only the *selection* of which works appear is curated again, not the image used once selected.
